...
objective is survival, formalised as the existence of a viable path from
...
criterion we call the \emph{sword}. A resource is a sword if and only if
it can actuate independently of the king \emph{and} the king can observe
it. This criterion is not chosen---it is the logical consequence
of the viability axiom under finite-bandwidth detection in a
multi-agent system with a unique principal agent
(\cref{sec:domain} delineates the boundary).

The framework yields several results that are surprising in their
precision. The lifecycle of every sword is binary: it is either
voluntarily relinquished (path~(a)) or forcibly eliminated (path~(b)).
There is no third option---attempting to ``prove your sword is not a
sword'' is a fixed-point impossibility. Unconstrained power is not
freedom but a perpetual elimination machine: the more control the king
has, the more swords he must cut.

The central thesis of this paper is that the sword is the
...
correctly classifies the fates of historical figures---Han~Xin (sword,
eliminated), Xiao~He (pre-sword, self-blunted), Zhang~Liang (not a sword,
...
\Cref{sec:framework} introduces the viability axiom, the differential
inclusion formalism, the Riemannian geometry of the viability kernel,
the sword definition, and the phase transition.
...
finite object.
Eleven appendices extend the analysis: historical
sources~(\cref{app:sources}), the complete puzzle
solution~(\cref{app:solution}),
case studies~(\cref{app:secondsex,app:threeli}),
a cooperative triad stability analysis~(\cref{app:triad}),
a bureaucratic lattice~(\cref{app:zhongxian}),
a sovereign debt absorption law~(\cref{app:govfi}),
a Gaokao reform analysis~(\cref{app:gaokao}),
a mean-field triviality theorem~(\cref{sec:alien}),
a three-body spectral damper~(\cref{sec:threebody}),
and a robotic manipulation instantiation~(\cref{sec:manipulation}).
