...
\State $c(e) \leftarrow c_{\max}(e)/2$ for all $e \in E$
  \Comment{init capacities}
\Statex
\For{\textcolor{sword}{\textbf{phase}} $\in$
  ...
  \Repeat
    \State $u \sim U$
      \Comment{sample command for current phase}
    \For{$t = 0, \ldots, H$}
      \Comment{\textcolor{water}{水: rollout}}
      \State $o_t \leftarrow \mathcal{E}.\texttt{sense}(q_t)$
        \Comment{IMU + encoders $\to \R^{42}$}
      \State $\tau_t \leftarrow \pi(o_t,\, u)$
        \Comment{policy: observation $\to$ torques}
    ...
    \State \textcolor{water}{\textsc{Evaluate}:}\;
      ...
      \min_{t}\bigl\{
      h(q_t)\!-\!h_{\min},\;\,
      \phi_{\max}\!-\!\|R_t\!-\!I\|_F\bigr\}$
      \Comment{\textcolor{water}{水: margin}}
    \State \textcolor{water}{\textsc{Gradient}:}\;
      $g \leftarrow
      \mathcal{E}.\texttt{grad}(|\phi|,\, c)$
      \Comment{\textcolor{water}{水: MJX autodiff}}
    \State $c(e) \leftarrow c(e)
      + \eta\bigl[\textcolor{water}{g_e}
      - \textcolor{knife}{\gamma\, c(e)}\bigr]$
      \Comment{Eq.~\eqref{eq:contactflow}}
    \State \textcolor{knife}{\textsc{Clamp}:}\;
      $c(e) \leftarrow \min\bigl\{c(e),\;
      c_{\max}(e)\bigr\}$
      \Comment{\textcolor{knife}{刀: enforce limits}}
    \If{$|\phi| \le 0$}
      \Comment{robot fell}
      \State \textcolor{sword}{\textsc{Reset}:}\;
        $\mathcal{E}.\texttt{reset}(\theta_0)$;\;\;
        $c \leftarrow c_{\max}/2$
...
\Ensure Trained policy
  $\pi \colon \R^{42} \to \R^{12}$
  \Comment{42-dim observation $\to$ 12 joint torques}
\end{algorithmic}
\end{algorithm}
