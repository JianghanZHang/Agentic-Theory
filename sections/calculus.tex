...
\begin{proof}
Assign edge capacities by the mean-field deviation:
for each agent~$r$ with bypass edge $e_r$, set
\[
  c(e_r) \;:=\; \max\bigl(\|U_r\| - \bar{U},\; 0\bigr).
\]
Under this assignment, only agents whose actuation exceeds the mean
carry positive bypass capacity; agents at or below the mean have
$c(e_r) = 0$ and are invisible to the cut.
...
The knife threshold is therefore the min-cut value under the
mean-field capacity assignment.
\end{proof}
...
\node[ref] at ($(knife.east)+(1.0,0.70)$)
