...
The preceding sections established the viability axiom, the sword
...
The central result: the sword is the min-cut, the viable path is the
max-flow, and ``the sword is the mean'' is max-flow/min-cut duality.

\section{The agentic space}\label{sec:tower}

The framework's objects organise into a four-level tower, each level
...
\textsc{Cut} is the sword: removing capacity from bypass edges.
...
to $\infty$ equals the minimum sword-cut capacity:
\[
  \max_\phi |\phi|
  \;=\;
...
\]
The sword threshold (\cref{thm:meanfield}) is the min-cut value.
The viable path (\cref{ax:viability}) is the max-flow.
``The sword is the mean'' $=$ max-flow equals min-cut.
\end{theorem}

\begin{proof}
By the capacity assignment lemma (\cref{lem:capacity}), each bypass
edge $e_r$ carries capacity
$c(e_r) = \max(\|U_r\| - \bar{U},\; 0)$.
Only agents whose actuation exceeds the mean carry positive bypass
capacity; agents at or below the mean have $c(e_r) = 0$ and are
invisible to the cut.

By the max-flow/min-cut theorem~\cite{diestel}, the maximum flow from
$\kappa$ to $\infty$ equals the minimum capacity of any
$\kappa$--$\infty$ cut.  The sword criterion (\cref{def:sword})
identifies bypass edges---edges with $c(e_r) > 0$ that do not pass
through $\kappa$.  The min-cut is the set of bypass edges whose
removal disconnects the graph; its value is
$\sum_{r \in C} c(e_r) = \sum_{r \in C} (\|U_r\| - \bar{U})$.
The detection threshold $\tau(\Obs)$ (\cref{thm:meanfield}) restricts
the king to edges with $c(e_r) > \tau$, so the operational min-cut
is the subset of bypass edges exceeding the detection threshold.
The sword threshold is therefore the min-cut value under the
mean-field capacity assignment (\cref{ax:massless,lem:capacity}).
\end{proof}
...
\[
  \textcolor{dao}{\text{抽刀}} \;\;
  \textcolor{dao}{\partial} \;\;
  \textcolor{water}{\text{水}} \;\;
  \textcolor{water}{\text{水}}\textcolor{sword}{\text{更流.}}
\]
\textsc{Cut} ($\textcolor{dao}{\partial}$, red) acts on flow
...
$\textcolor{dao}{\text{刀}}$ to $\textcolor{water}{\text{水}}$.
\end{remark}
...
\begin{itemize}
  \item \textbf{刀} (sword $= \partial$, boundary operator):
  ...
  along a free edge. Water flows where the sword does not cut.
\end{itemize}
\end{definition}
...
\begin{enumerate}
  ...
  \item \textbf{Sword detection.} Flag bypass edges where
...
\begin{tikzpicture}[
  ...
  % ── sword (刀) node: red ──
  knf/.style={diamond, draw=dao, thick, aspect=2.5,
    fill=dao!6, minimum width=1.2cm, align=center, font=\small,
    inner sep=1pt},
  % ── phase (青冥) nodes: cyan ──
...
\node[knf, below=1.1cm of sgd] (dao)
  {6.\ $\textcolor{dao}{c(e) > \bar{c}{+}\tau}$\,?};

\node[phs, right=of dao] (cut)
  {7.\ \textcolor{dao}{\textsc{Cut} $\partial$} /
  \textcolor{sword}{\textsc{Phase} $\varphi$}};

\node[phsd, below=1.1cm of dao] (viable)
...
% dao: above-right (clear of spine)
\node[ref] at ($(dao.east)+(1.0,0.70)$)
...
\textcolor{dao}{Red} (刀): sword detection
...
(Thm~\ref{thm:flowcut}).
The loop is 抽刀断水水更流: \textcolor{dao}{cut}
...
Sword detection (step~6) identifies precisely such edges via the
...
The Cheeger constant $h(G)$ is the \emph{normalised sword threshold}.
...
\textcolor{dao}{The sword is the cut.}
...
\begin{itemize}
  ...
  \item \textbf{\textcolor{dao}{Physics.}}
...
dissipation term supplied by the sword. This distinction is the
difference between symplectic mechanics (energy conserved) and contact
mechanics (energy dissipates). The sword is the dissipation.
...
\begin{definition}[Sword dissipation]\label{def:dissipation}
The \emph{sword function} $\gamma \colon E \to \R_{\geq 0}$ is
\[
  \gamma(e) \;=\;
  \begin{cases}
...
\begin{equation}\label{eq:contactflow}
  ...
  {\textcolor{dao}{\text{刀: dissipation}}}.
\end{equation}
...
decreases capacity on flagged edges (sword detection + \textsc{Cut},
...
\begin{itemize}
  ...
  \item $\textcolor{dao}{\gamma(e)\,c(e)}$: the sword pushes capacity
...
$\textcolor{dao}{\text{刀}}$ in the dissipation,
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item \textbf{\textcolor{dao}{Activate}}:\;
  ...
  in $O(1)$ time.
  \textsc{Cut}: the sword at the neuron level.
\end{enumerate}
One layer:
$z_\ell = \textcolor{dao}{\mathrm{ReLU}}\bigl(
...
$\textcolor{dao}{r} = \mathrm{ReLU}$.

\textcolor{water}{水} $=$ linear transport ($W, b$).
\textcolor{dao}{刀} $=$ ReLU gate
...
\end{enumerate}
$\textcolor{dao}{刀}$ at the neuron scale (hard gate).
...
\Function{\textsc{Forward}}{$o,\, u$}
  ...
  \For{$\ell = 1, \ldots, L{-}1$}
    ...
    \State \textcolor{dao}{\textsc{Activate}:}\;
      $z_\ell \leftarrow \max(0,\, y)$
      \Comment{\textcolor{dao}{刀: ReLU (Cut)}}
  \EndFor
...
\Function{\textsc{Backward}}{$\delta\tau$}
  ...
  \For{$\ell = L{-}1, \ldots, 1$}
    \State \textcolor{dao}{\textsc{Activate}${}^T$:}\;
      $\delta y \leftarrow \delta z \odot
      \mathbf{1}_{y_\ell > 0}$
      \Comment{\textcolor{dao}{刀: ReLU mask (same Cut)}}
  ...
  \Repeat
    ...
    \For{$t = 0, \ldots, H$}
      ...
      \State $q_{t+1} \leftarrow \mathcal{E}.\texttt{step}(q_t,\, \tau_t)$
        \Comment{EL dynamics (\textcolor{dao}{no $\nabla$})}
    \EndFor
    ...
    \State $W_\ell \leftarrow W_\ell
      ...
      - \textcolor{dao}{\gamma\, W_\ell}\bigr]$;\;\;
      $b_\ell \leftarrow b_\ell
      + \eta\,\textcolor{water}{\nabla b_\ell}$
      \Comment{Eq.~\eqref{eq:contactflow} on $\pi$}
    \State \textcolor{dao}{\textsc{Clamp}:}\;
      $\|W_\ell\|_F \leftarrow
      \min\bigl(\|W_\ell\|_F,\;
      c_{\max}(e_\ell)\bigr)$
      \Comment{\textcolor{dao}{刀: enforce capacity}}
    \If{$|\phi|_\beta \le 0$}
      \Comment{robot fell}
...
objective: $|\phi| > 0$ (do not fall). The sword
...
\textcolor{dao}{black box}: the gradient does not penetrate
the physics.
...
In any agentic space with sword dissipation $\gamma > 0$
...
\begin{remark}[The superconducting exception]\label{rem:superconductor-exception}
The regime $\gamma = 0$ with $\lambda_1 > 0$ maintained by a spectral
gap lies outside the hypothesis of \cref{thm:camus} ($\gamma > 0$).
This is the superconducting state: dissipation vanishes but stability
persists because the gap prevents low-energy excitations from disrupting
coherent transport.  See \cref{rem:3b-superconductor} for the physical
mechanism.
\end{remark}
...
sword dissipation (\cref{def:dissipation}).
\end{definition}
...
the same equation, the same flow, the same sword---on the dual tower.
\smallskip
\end{minipage}}
\medskip
...
The sword between Subject and Other is the mean
...
\For{\textcolor{sword}{\textbf{phase}} $\in$
  ...
  \Repeat
    ...
    \State $W_\ell^{m} \leftarrow W_\ell^{m}
      + \eta\bigl[\textcolor{water}{\nabla W_\ell^{m}}
      - \textcolor{dao}{\gamma\, W_\ell^{m}}\bigr]$;\;\;
    ...
    \State \textcolor{dao}{\textsc{Clamp}:}\;
      ...
      c_{\max}(e_\ell)\bigr)$
      for all $\ell$
      \Comment{\textcolor{dao}{刀: capacity}}
...
and the rank~$r = c_{\max}(e_\ell^{\mathrm{task}})$ is the sword---it
...
\draw[arr, dao]  (upd.east) -- ++(1.3,0) |-
  node[right, pos=0.25, font=\scriptsize, color=dao]{repeat}
  (fwd.east);
...
\If{$\phi_{\mathrm{topo}} = 0$}
  \textcolor{dao}{\Return} $|\phi|_{\mathrm{code}} = 0$
  \Comment{\textcolor{dao}{killed}: does not parse}
\EndIf
\Statex
...
\If{$\phi_{\mathrm{safe}} = 0$}
  \textcolor{dao}{\Return} $|\phi|_{\mathrm{code}} = 0$
  \Comment{\textcolor{dao}{killed}: runtime crash}
\EndIf
\Statex
...
\textcolor{dao}{killed} before the judge sees it.
...
\State \textcolor{dao}{\textsc{Destroy}}:\;
  $\mathcal{C}.\texttt{kill}()$;\;\;
  reclaim all resources
  \Comment{\textcolor{dao}{no persistent state}}
\Statex
...
\textsc{Destroy}
  ...
  & kill container \\
\midrule
\textcolor{dao}{Isolation}
  & joint limits, ground plane
  & no network, read-only fs \\
\textcolor{dao}{Gravity}
  & $9.81\;\mathrm{m/s^2}$
  & time limit $t_{\max}$ \\
\textcolor{dao}{Ground contact}
  & collision detection
  & memory limit $m_{\max}$ \\
\textcolor{dao}{$c_{\max}$}
  & joint torque bound
...
The \textcolor{dao}{sword} is the same.
\end{remark}
...
The \textsc{Cut} operation $\textcolor{dao}{\partial}$ is
\emph{not} an instruction: it is a constraint imposed by the
environment (the sword), not an action taken by the agent.
\end{definition}

\begin{remark}[The sword cannot cut itself]
\label{rem:sword-self}
The exclusion of $\textcolor{dao}{\partial}$ from~$\mathcal{I}$
...
The sword is not a tool the agent wields---it is the boundary the
agent lives inside.
...
The program that removes its own sword
\textcolor{dao}{destroys itself}.
...
\\[4pt]
\hspace*{2em}%
\textcolor{dao}{You cannot use the system to prove the system
safe.}
\\[4pt]
The \textcolor{dao}{sword} is the price of
\textcolor{water}{flow}.
...
\textcolor{dao}{\textsc{Destroy}} from the per-evaluation
loop:
\[
  \underbrace{%
