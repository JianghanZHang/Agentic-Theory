...
internal sword dynamics secondary & Mongol invasion, colonialism \\
...
Universal $\Ur \approx \varnothing$ & No swords; framework trivial &
...
The framework so far analyzes one layer: the king--sword interaction. A
...
swords, administer territory). The pawn needs water to function. If
...
one. The pawn becomes a sword---$\Ur$ transitions from $\varnothing$ to
$\neq \varnothing$---and the king faces a sword he created by exhausting
the water.
\end{proposition}
...
language: the states depleted their own water, creating the swords that
destroyed them. Qin, having unified, then depleted its own water
(conscription for the Great Wall and Epang Palace), creating the swords
(Chen Sheng, Wu Guang) that destroyed it.

\begin{remark}[Irreversibility of the pawn-to-sword transition]
\label{rem:dumu-irreversibility}
The hypothesis ``$w(t)$ decreasing'' is not merely a sufficient
condition: the transition is hysteretic.  Once $\Ur$ expands from
$\varnothing$ to $\{\text{submit}, \text{rebel}\}$, the organisational
infrastructure of rebellion (networks, leaders, arms) persists even
if $w$ is subsequently restored.  The causal envelope
$\mathrm{Reach}(x, \Ur, T)$ ratchets: it can expand but not contract,
because the agent's Bayesian update---having observed the king drive
$w \to 0$ once---permanently raises the perceived probability of
recurrence.  Restoration of $w$ reduces the \emph{incentive} to
rebel but not the \emph{capability}; the pawn remains a sword by
condition~(1) of \cref{def:sword}.
\end{remark}
...
$\implies$ pawn $\to$ sword $\implies$ no path to $\infty$).
\end{remark}
