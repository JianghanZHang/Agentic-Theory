\chapter{华容道: Complete Instantiation}\label{sec:huarongdao}
...
configuration 横刀立马 (``horizontal sword, standing horse''):
...
Sword (\cref{def:sword}) & 关羽 $(2{\times}1)$ & Blocks exit;
...
\emph{Sword.} 关羽 satisfies both conditions of \cref{def:sword}:
...
The configuration's traditional name means ``horizontal sword, standing
horse.'' 刀~(sword) $=$ 关羽 blocking horizontally; 马~(horse) $=$
...
关羽 has both conditions of \cref{def:sword}: autonomous actuation
...
The puzzle's $2 \times 2$ classification is formalised in
...
\section{The experiment}\label{sec:kpc}
...
The sword ($2 \times 1$) requires mode~H\@.
In mode~S, only soldiers act---水 is diffuse.
\end{definition}

\subsection*{The counting: 81}
...
华容道 solutions uses~步, not unit moves.
...
\begin{enumerate}[label=(\roman*)]
  ...
  yield $81$~步.
  \item Along the 步-optimal path (81~步):
  $118$~unit moves, of which $37$ are multi-slide steps
  (same piece slides $\geq 2$ cells).
  Note: the unit-move-optimal path requires only $116$~unit
  moves (\cref{rem:counting}); these are different
...
This \emph{is} 义释曹操 (\cref{rem:guanyu}): 水 must
...
\subsection*{Saddle and mirror descent}
...
\begin{enumerate}[label=(\roman*)]
  \item 关羽 (sword) is at position $(2, 0)$, far from
...
\end{enumerate}
To cross the saddle is to perform 义释曹操
(\cref{rem:guanyu}).
\end{theorem}

\begin{proof}
By \cref{thm:flowcut}, the min-cut separates
blocked configurations (关羽 covers exit corridor)
...
at $\sigma^*$, 关羽 has exited the corridor.
\end{proof}
...
\midrule
H (horizontal) & $27$ & sword, king (horizontal) \\
...
\subsection*{The isomorphism, completed}
...
\begin{table}[H]
\centering\small
\caption{Agentic calculus on 华容道: complete correspondence.}%
\label{tab:kpc}
\begin{tabular}{@{}lll@{}}
\toprule
