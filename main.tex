...
\definecolor{dao}{RGB}{180,30,30}        % 刀 — red
...
{\LARGE\bfseries The Sword Is the Mean}\\[8pt]
{\Large Mathematical Principles of Viability Maintenance\\
under Temporal Linearity}

\vspace{1.5cm}

{\large 有志者事竟成，\textcolor{dao}{破釜沉舟}\textcolor{dao}{百二秦关}\textcolor{sword}{终属楚}；\\[3pt]
...
whose Cheeger constant is the sword.
Under this axiom, we derive a structural criterion---the
\emph{sword}---that classifies any resource as a threat based on two
conditions: autonomous actuation and observability.  We prove that the
sword is not an intrinsic property of a resource but a \emph{phase
function} determined by the system's mean field.  We establish three
main results: (1)~a binary lifecycle theorem (every sword is either
...
these results into a flow-theoretic language: the sword is the min-cut
of the execution graph, the viable path is the max-flow, and the
central thesis---the sword is the mean---is a restatement of
...
topologies, and militarism.  The central thesis is that the sword is the
...
\input{sections/meanfield}
\input{sections/applications}
\input{sections/discussion}
\input{sections/calculus}
\input{sections/huarongdao}
\input{sections/conclusion}

% ── Appendices ────────────────────────────────────────────
\appendix
\input{appendices/sources}
\input{appendices/solution}
\input{appendices/second_sex}
\input{appendices/three_li}
\input{appendices/triad}
\input{appendices/zhongxian}
\input{appendices/govfi}
\input{appendices/gaokao}
\input{appendices/fields2022}
\input{appendices/threebody}
\input{appendices/manipulation}
\input{appendices/catching}
...
in \emph{IEEE/RSJ Int.\ Conf.\ on Intelligent Robots and Systems},
2012, pp.~5026--5033.
\bibitem{pdp}
W.~Jin, Z.~Wang, Z.~Yang, and S.~Mou,
Pontryagin differentiable programming: An end-to-end learning and
control framework,
in \emph{Advances in Neural Information Processing Systems}
\textbf{33}, 2020, pp.~7979--7992.
\bibitem{mason}
M.~T.~Mason,
\emph{Mechanics of Robotic Manipulation},
MIT Press, 2001.
\bibitem{cheng-manip}
X.~Cheng, E.~Huang, Y.~Hou, and M.~T.~Mason,
Contact mode guided motion planning for quasidynamic dexterous
manipulation in 3D,
in \emph{IEEE Int.\ Conf.\ on Robotics and Automation}, 2022,
pp.~6520--6526.
\bibitem{contactsdf}
Y.~Wen, X.~Cheng, G.~Izatt, and R.~Tedrake,
ContactSDF: Signed distance functions as multi-contact models for
...
Springer, 2005 (reprint of the 1985 original).
\end{thebibliography}
