...
For the parallel-jaw geometry of \cref{ex:parallel-jaw}, each
finger contacts a vertical face of the box; the contact normals
are horizontal (mutually opposing along the grasp axis), and the
two-dimensional tangential plane at each contact contains the
vertical direction. Gravity therefore lies fully in the
tangential plane and must be supported by friction. Let
$\|\cdot\|_{t}$ denote the tangential component along the
fixed-direction (vertical-plus-wind) tangential axis at the
contact, and let $f_{t,L}$, $f_{t,R}$ denote the magnitudes of
the tangential friction forces produced by the left and right
fingers along that axis. By the symmetry assumption
$f_{t,L} = f_{t,R} = f_{t}$, so the total tangential force
delivered to the box is $2 f_{t}$.

Project the Newton balance \eqref{eq:newton-balance} onto the
tangential axis. The right-hand side
$m(\ddot{x}_{\mathrm{des}} - g) + \Fwind$, projected
tangentially, has three contributions: the inertial component
$m\,\|\ddot{x}_{\mathrm{des},\, t}\|$, the gravity-support
component $m\,\|g\|$ (gravity is fully tangential in this
geometry), and the wind component $\|\Fwind(t)\|_{t}$.
Worst-casing the alignment of inertial, gravity, and wind
contributions along the tangential axis gives the budget
\begin{equation}
\label{eq:ft-projection}
  ...
  \;+\; \|\Fwind(t)\|_{t}.
\end{equation}
The friction-cone constraint $|f_{t}| \le \mu f_{n}$ at each
finger, together with the per-finger normal-force budget
$f_{n} \le f_{n,\max}$ (a hardware ceiling on the gripper
actuator), gives $2|f_{t}| \le 2 \mu f_{n,\max}$. Combining
with \eqref{eq:ft-projection} and rearranging bounds the
tangential acceleration:
\begin{equation}
\label{eq:accel-bound-derivation}
  m \,\|\ddot{x}_{\mathrm{des},\, t}\|
  \;\le\;
  2\, \mu \, f_{n,\max} \;-\; m\,\|g\| \;-\; \|\Fwind(t)\|_{t}.
\end{equation}
Dividing by $m$ yields the explicit form of $\alpha$
promised in \eqref{eq:accel-bound}. The dependence on the
configuration $\jointvec$ enters through the orientation of
the contact normal in the world frame, which determines how
much of the wind force projects into the tangential direction.
...
budget $f_{n,\max}$ per finger, gravitational acceleration of
magnitude $\|g\|$, and wind force $\Fwind(t)$, the acceleration
bound \eqref{eq:accel-bound} of \cref{prop:bilevel-coupling}
...
\label{eq:alpha-explicit}
  ...
  \frac{2\, \mu\, f_{n,\max}
        \;-\; m\,\|g\|
        \;-\; \|\Fwind(t)\|_{t}(\jointvec)}{m},
\end{equation}
where $\|\cdot\|_{t}$ denotes the tangential component along
the (vertical-plus-wind) tangential axis at the contact, and
$\|\Fwind(t)\|_{t}(\jointvec)$ is the configuration-dependent
projection of the wind force into that axis.
\end{proposition}

\begin{proof}
By symmetry, the two fingers each apply a tangential friction
force of magnitude $f_{t}$, with total tangential force on the
box equal to $2 f_{t}$. Project the Newton balance
\eqref{eq:newton-balance} of \cref{sec:bilevel} onto the
tangential axis: the right-hand side $m(\ddot{x}_{\mathrm{des}}
- g) + \Fwind$, projected tangentially, contributes
$m \|\ddot{x}_{\mathrm{des},\, t}\| + m \|g\| +
\|\Fwind\|_{t}$ in the worst-case alignment, giving
\eqref{eq:ft-projection}. The per-finger friction-cone
constraint $|f_{t}| \le \mu f_{n}$ combined with the per-finger
normal-force ceiling $f_{n} \le f_{n,\max}$ yields
$2|f_{t}| \le 2\mu f_{n,\max}$; substituting into
\eqref{eq:ft-projection} and rearranging gives
\eqref{eq:alpha-explicit}. The configuration dependence enters
through $\|\Fwind(t)\|_{t}(\jointvec)$, which depends on the
orientation of the contact normal at $\jointvec$. As a
consistency check, setting $\ddot{x}_{\mathrm{des}} = 0$ in
\eqref{eq:accel-bound-derivation} reproduces the static-grasp
slipping threshold $2\mu f_{n,\max} = m\|g\| +
\|\Fwind\|_{t}$ that defines $\rcontact_{\mathrm{grasp}} = 1$
in the wind-perturbed extension \eqref{eq:rho-grasp-wind} of
\cref{ex:parallel-jaw}.
\end{proof}
