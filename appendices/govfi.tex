...
\textcolor{dao}{red} = corruption/attack,
...
$\beta_i$         & RG $\beta$ function $= r - k_i$
                  & 重正化群 $\beta$ 函数 \\
$\mathcal{K}$     & Damping matrix $= \operatorname{diag}(k_0, k_1, k_2)$
                  & 阻尼矩阵 \\
$M$               & Stability matrix $= \mathcal{K} - rI$
                  & 稳定矩阵 \\
\midrule
\multicolumn{3}{@{}l}{\emph{Three-layer graph / 三层图}} \\
Layer 0           & \textcolor{water}{政府 (Gov)} --- source, issues bonds
                  & 源节点, 发行债券 \\
Layer 1           & \textcolor{dao}{公司 (Companies)} --- min-cut
                  & 最小割, 路由资金+信息 \\
...
two sides}.
\textcolor{dao}{Corruption} is the
\textcolor{dao}{duality gap}.
...
\textcolor{dao}{corrupted}
at \textcolor{dao}{Layer~1} (\cref{ex:three-layer}),
complementary slackness fails
and a \textcolor{dao}{\emph{duality gap}} opens:
\begin{equation}\label{eq:fiscal-duality-gap}
  ...
  \;\geq\; 0.
\end{equation}
The loss field $\textcolor{dao}{\mathcal{L}(t)}$
\emph{is} the \textcolor{dao}{duality gap}.
...
\textcolor{water}{0}
  ...
  & \textcolor{water}{Dual} \\
\textcolor{dao}{1}
  & \textcolor{dao}{公司 (Companies)}
  & Mapping $\lambda \mapsto C$
  & \textcolor{dao}{Coupling} \\
\textcolor{water}{2}
...
\medskip\noindent
\emph{\textcolor{dao}{Corruption $=$ duality gap $> 0$.}\quad
...
% ================================================================
\section{Sovereign debt as sword}\label{sec:gf-sword}
% ================================================================
...
\begin{proposition}[Debt satisfies the sword definition]
\label{prop:debt-sword}
Sovereign debt $D(t)$ satisfying \eqref{eq:debt-dynamics}
is a \textcolor{dao}{sword} in the sense of
\cref{def:sword}:
\begin{enumerate}[label=(\roman*)]
...
(\cref{rem:sword-di}).  Condition~(ii) is structural:
...
is a self-reinforcing triangle: the sword actuates itself,
generates the conditions for further actuation, and
compounds without external input.
\end{remark}

\begin{corollary}[Sword lifecycle for debt]
\label{cor:debt-lifecycle}
By \cref{thm:lifecycle}, every sword admits exactly two
resolution paths:
\begin{enumerate}[label=(\alph*)]
  \item \textbf{Controlled restructuring} (path~(a) of
    ...
    growth, relinquishing the sword before it becomes
    lethal.
...
but a deferral that preserves the sword's autonomous
...
\begin{enumerate}[label=(\alph*)]
  ...
  \item for $p > p_c$, the expected cascade is
    ...
    probability.
\end{enumerate}
The percolation threshold is a second sword-is-the-mean
...
\begin{proposition}[Full observability eliminates hidden swords]
\label{prop:govfi-observability}
Under full observability ($\Obs = \mathcal{F}$), no debt
instrument can satisfy the sword definition
(\cref{def:sword}) \emph{without detection}.  By
\cref{prop:imperfect}, the gap between autonomous actuation
and detected actuation is
$\|U(x)\| - \|U(x) \cap \Obs(x)\|$.  Full observability
closes this gap to zero: every sword is visible the moment
it forms.
...
observability that eliminates hidden swords
...
debt from a \textcolor{dao}{sword} (autonomous,
...
\textcolor{dao}{1} & \textcolor{dao}{公司 (Companies)}
  & \textcolor{dao}{截留}: receive $B$, pass $<B$ to builders
...
\textcolor{dao}{L1 公司}  & 0亿  & \textcolor{dao}{10亿}
  & \textcolor{dao}{Gamed}: $\mathcal{L} = 0$
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item \textcolor{dao}{\textbf{Layer~1 survives:}}
    ...
    \textcolor{dao}{\textbf{唯一可持续攻击面.}}
\end{enumerate}

\noindent
\textbf{Conclusion.}
Corruption concentrates at the \textcolor{dao}{min-cut (Layer~1)} by structural
...
\textcolor{water}{L0} & 截留 (self-div)
  ...
  & \textcolor{water}{\textbf{行不通}} \\
\textcolor{dao}{L1} & \textcolor{dao}{截留 (diversion)}
  & 0亿 & \textcolor{dao}{10亿}
  & \textcolor{dao}{Yes (min-cut)}
  & \textcolor{sword}{独立验收}
  & \textcolor{dao}{\textbf{Unique attack}} \\
...
\textcolor{dao}{Layer~1 is the unique sustainable attack surface} because
it is the only layer that controls both the
\textcolor{dao}{information} and
\textcolor{dao}{money} channels of the fiscal flow graph.
\end{example}
...
\begin{enumerate}[label=(\alph*)]
  ...
  \item \textcolor{water}{\textbf{Grace margins.}}
    ...
    $\ln 2\,/\,(2\epsilon_{\min})$.
  \item \textcolor{dao}{\textbf{Failure without GovFi.}}
    If \textcolor{dao}{$k_1 = 0$} (no verification at the min-cut), then
    $\dot{V}$ contains the \textcolor{dao}{positive} term
    \textcolor{dao}{$r\,\mathcal{L}_1^2$}: the Lyapunov condition fails.
...
\textcolor{dao}{1} & \textcolor{dao}{合理利润 (legitimate overhead)}
...
\textcolor{water}{0} & \textcolor{water}{正处/副处 (county)}
  ...
  & 公款吃喝, 招待费 \\
\textcolor{dao}{1} & \textcolor{dao}{正科级 (section)}
  & \textcolor{dao}{Runs SOEs, controls min-cut}
  & 管理费, 合理利润 \\
...
\label{rem:govfi-conservation}
The \textbf{loss identity} (telescoping through the three layers) is
\[
  \mathcal{L}_{\textup{total}}
  \;=\; B_{\textup{iss}} - C_{\textup{ver}}.
\]
This is conservation: every yuan either reaches verified construction
or is accounted for as loss.  The code checks four properties:
\begin{enumerate}
  \item \emph{Monotone flow}: $B_{\textup{iss}} \ge B_{\textup{dis}}
        \ge C_{\textup{ver}}$ (money flows downhill).
  \item \emph{Non-negativity}: all quantities $\ge 0$.
  \item \emph{Loss decomposition}:
        $\mathcal{L}_{\textup{total}} = \mathcal{L}_{\textup{bond}}
        + \mathcal{L}_{\textup{exec}}$ (loss splits into two layers).
  \item \emph{Loss identity}:
        $\mathcal{L}_{\textup{total}} = B_{\textup{iss}} - C_{\textup{ver}}$
        (the telescoping sum).
\end{enumerate}
The ``leak'' is
$\ell = \mathcal{L}_{\textup{total}} - (B_{\textup{iss}} - C_{\textup{ver}})$.
A non-zero~$\ell$ would indicate a bookkeeping error in the ledger.
The simulation passes with $\ell = 0$
(verified by \texttt{python -m govfi.simulate}).

\textcolor{sword}{%
This is the same structure as the student allocation check
(\cref{rem:gaokao-conservation}):
$\text{leak} = \sum\text{inflow} - \sum\text{outflow}$.
Money flows through fiscal layers the way students flow through
provinces---conservation holds in both.}
\end{remark}
...
With the damping matrix $M = \mathcal{K} - rI = \operatorname{diag}(\epsilon_0,
...
1 & \textcolor{dao}{公司 (Companies)}
  & \textcolor{dao}{Coupling} (min-cut):
...
\begin{tikzpicture}[
  ...
  leak/.style={rectangle, rounded corners=4pt,
    draw=dao, thick, dashed, fill=dao!6,
  ...
  darr/.style={-{Stealth[length=6pt]}, thick, dashed, dao},
  ...
  \node[leak] (corrupt) at (3.8, -6.2)
    {\textcolor{dao}{隐性流失}\\[-2pt]
     \textcolor{dao}{\scriptsize hidden leak}};
...
The \textcolor{dao}{min-cut} is at Layer~1 (Companies),
...
\node[dao, anchor=west, font=\footnotesize\bfseries]
  at (11, 7.5) {Without GovFi};
\node[dao, anchor=west, font=\footnotesize]
  at (11, 5.8) {(open-loop)};
...
\node[dao, font=\footnotesize, anchor=south] at (15, 1.5)
  {$t \to\,$?};
...
\textcolor{dao}{Without GovFi} (dashed): open-loop,
...
gap $\|\Obs\| > 0$ hides the sword
(\cref{def:sword}, condition~(2)).
...
\begin{remark}[The 城投 sword]\label{rem:chengtou}
China's local government financing vehicles
(地方政府融资平台, commonly called 城投) are the
current instantiation of the fiscal sword.  The
...
The sword is real.  The losses are real.  The only
...
$K_F$.
The sword-is-the-mean theorem (\cref{thm:meanfield})
...
a tool (financing productive investment) or a sword
