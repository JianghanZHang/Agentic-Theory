...
蟠桃宴 (Peach Banquet): another relabelling.
He rebels again.
...
\Cref{thm:fixedpoint} predicts: relabelling preserves
...
This is the acoustic version of ``the sword is the mean'': whether
...
\paragraph{Why smooth: factorisation.}
The war-hero's contribution \emph{factorises}: it can be evaluated
...
\begin{definition}[Factorised contribution]\label{def:factorized}
An agent's contribution $c \in \mathbf{L} \oplus \mathbf{L}^*$ is
\emph{factorised} if it decomposes as
...
contribution is factorised (\cref{def:factorized}).
...
If $c$ is not factorised, then $c_{\mathbf{L}}$ enters the evaluation
...
\Cref{thm:shangyang} describes delocalisation for pure contributions
(norm fully conserved).
\Cref{rem:caijing} describes the entangled case (system discards both
components).
Factorised contributions produce a third mode:
...
Entangled & $c_+,\, c_-$ shared support &
  ...
  蔡京 \\
Factorised & $c_{\mathbf{L}} \otimes c_{\mathbf{L}^*}$ &
...
\begin{remark}[Smooth $=$ no phase change]\label{rem:smooth-phase}
Smoothness of the recognition path (\cref{prop:smooth-li}) is
equivalent to absence of phase transition in the representation
dynamics.  When the contribution factorises, the contact Hamiltonian
separates:
\[
  H(\mathbf{L},\, \mathbf{L}^*)
  \;=\;
  H_{\mathbf{L}}(\mathbf{L}) \;+\; H_{\mathbf{L}^*}(\mathbf{L}^*).
\]
The $\mathbf{L}^*$ flow is autonomous: it does not couple to
$\mathbf{L}$.  When $\mathbf{L}$ changes discontinuously (execution,
exile, death), the $\mathbf{L}^*$ trajectory receives no kick---there
is no cross-term $\partial^2 H / \partial\mathbf{L}\,
\partial\mathbf{L}^* = 0$.  No coupling, no kick, no phase transition.

辛弃疾 stated the separation condition in 青玉案$\cdot$元夕:
\begin{quote}
众里寻他千百度，蓦然回首，那人却在，灯火阑珊处。

\medskip
\emph{A thousand times I searched for her in the crowd.\\
Suddenly I turned---and there she was,\\
in the dim lantern light.}
\hfill ---辛弃疾，青玉案$\cdot$元夕
\end{quote}
The search ``众里寻他千百度'' is motion along $\mathbf{L}$: political
manoeuvring, seeking power, traversing the crowd.  It never finds
$\text{丽}^*$, because $\text{丽}^*$ is not on $\mathbf{L}$.
The ``回首'' (turning) is the projection from $\mathbf{L}$ to
$\mathbf{L}^*$---a reorientation, not a phase transition.
And ``灯火阑珊处'' (the dim lantern light) is the $\mathbf{L}^*$
region that was always there, unexamined.  $\text{丽}^*$ is found not
by searching further on $\mathbf{L}$ but by recognising that the
...
Both authors are displaced swords: agents with 力 (capability) whom
...
\item Cut vertex removed (total collapse): the novel ends in
  tragedy.
  水浒传's 招安 is path~(c)---relabelling 108 outlaws as loyal
...
$\mathbf{L}^*$ with the same equation, the same sword, the same
...
delocalisation, nothing is conserved
...
This produces a delocalisation problem absent from
\cref{thm:shangyang}.
商鞅 and 王安石 have pure contributions: upon delocalisation, norm is
fully conserved.
