...
\textcolor{sword}{the sword is the mean}.
...
``the sword is the mean'' (\cref{thm:meanfield}).  The duality
...
\textcolor{dao}{no intermediate phase}.
...
\[
  ...
  \begin{cases}
    0 & \text{if } p < p_c \quad
    \text{\emph{(\textcolor{dao}{eliminated})}}, \\
    > 0 & \text{if } p > p_c \quad
...
Agents execute in physical spacetime
$\mathbb{R}^3 \times \mathbb{R}^1 = \mathbb{R}^4$, where
$\mathbb{R}^1$ is the irreversible wall-clock time of
\cref{rem:di-temporal,rem:temporal}.  Each execution path
(worldline of an autonomous agent) is a curve in $\mathbb{R}^4$
with Hausdorff dimension~$2$ (Brownian scaling under
finite-bandwidth detection); the intersection criterion
$2 + 2 \geq d$ is marginal at $d = 4$
(\cref{rem:d4-not-analogy}).  At the upper critical dimension
$d_c = 4$:
\[
  \textcolor{sword}{%
  \text{the sword is the mean}
  \;\;\xrightarrow{\;d = 4\;}\;\;
...
\begin{remark}[Why $d = 4$ is not an analogy]\label{rem:d4-not-analogy}
The Hausdorff dimension intersection criterion is a topological
theorem about generic intersection in~$\mathbb{R}^d$; it does not
distinguish between spatial and temporal dimensions.  Two
independent execution paths (worldlines of autonomous agents)
are curves in physical spacetime
$\mathbb{R}^3 \times \mathbb{R}^1 = \mathbb{R}^4$.  Each path
has Hausdorff dimension~$2$ (Brownian scaling under
finite-bandwidth detection).  The intersection probability
transitions from recurrent ($d < 4$) to transient ($d > 4$) at
$d = 4$.

The temporal dimension is not Wick-rotated fictitious time:
it is the irreversible wall-clock time of
\cref{rem:di-temporal,rem:temporal}.  Agents execute in
spacetime, not in space.  Their causal envelopes
(\cref{eq:causal-envelope}) are subsets of~$\mathbb{R}^4$, not
$\mathbb{R}^3$.  The dimension count $2 + 2 = 4$ is a
statement about the ambient dimension of the execution space,
which is physical spacetime.
\end{remark}
...
\begin{itemize}
  ...
  \item \textcolor{dao}{Cuts} $= \mathrm{im}(\partial_2^*)$:
...
to eliminating all swords by cutting edges.
\end{proposition}
...
\begin{remark}[Expand-or-compress $=$ sword-or-not-sword]
\label{rem:alien-gcd}
...
Expand $=$ viable.  Bottleneck $=$ sword, which the compression step
eliminates.  The iteration terminates because each compression reduces
the graph's complexity: the number of swords is finite, and the binary
...
(\cref{def:sword}): the sieve observes which elements are composite
...
\begin{proposition}[Self-duality $=$ the sword is the mean]
\label{prop:selfdual-sword}
\leavevmode
\begin{enumerate}[label=(\roman*)]
...
the interaction.  \textcolor{sword}{The sword is the mean---the
...
where the sword equals the mean.
...
Tool &
  ...
  modular forms \\
\midrule
\textcolor{dao}{Binary lifecycle} &
  sharpness &
  --- &
  0--1 law &
  --- \\
\textcolor{sword}{Sword $=$ mean} &
  $p_c = p_{\mathrm{sd}}$ &
  Poincar\'e duality &
  --- &
  $E_8 = E_8^*$ \\
...
\begin{enumerate}[label=(\roman*)]
  ...
  ``The sword is the mean'' is a theorem.
  ...
  optimal.
\end{enumerate}
Together: \textcolor{sword}{the sword is the mean, exactly, sharply,
...
\textcolor{dao}{The alien model cannot prove
$\mathrm{P} \neq \mathrm{NP}$.}
...
\begin{remark}[The gap as superconducting gap]\label{rem:gap-superconductor}
The spectral gap $\lambda_1 > 0$ that separates $\mathrm{P}$ from
$\mathrm{NP}$-hard is structurally identical to the superconducting
gap $\Delta > 0$ in BCS theory: both prevent low-energy excitations
from disrupting coherent transport.
See \cref{rem:3b-superconductor} for the physical mechanism
(vibrational friction averaging) and the identification
$\lambda_1 \leftrightarrow \Delta$.
\end{remark}
...
\noindent
\textcolor{sword}{%
有志者事竟成，\textcolor{dao}{破釜沉舟}百二秦关终属楚；\\
