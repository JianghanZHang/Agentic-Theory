\chapter{原典与人物}\label{app:sources}

The historical analysis in this paper draws primarily on Sima Qian's
\emph{Shiji} \cite{shiji} (《史记》, \emph{Records of the Grand Historian},
c.~94~BCE) and Du Mu's \emph{A Fang Gong Fu} \cite{dumu} (《阿房宫赋》, 825~CE).
...
in household production or labour.}
\hfill ---《史记·高祖本纪》
\end{quote}
...
strategise. His sole structural role was as the mandatory routing node.
...
than any other general in the Chu--Han war. Pure sword
...
so the sword cannot be cut yet. Granting the title $=$ extending the
horizon. After the phase transition, the sword was cut.
...
Bang's campaigns. Sword who regressed to pre-sword
(\cref{def:presword}) via deliberate self-corruption
(\cref{ex:xiaohe}).
...
coordinate a revolt. This is a sword $\to$ pre-sword regression
(\cref{def:presword}): condition~(2) is suppressed by pulling
$\|\Ur\|$ toward $\bar{U}$ (\cref{thm:meanfield}), but
condition~(1) persists---the grain supply remains under his control.
...
Not a sword (\cref{ex:zhangliang}): pure advisory function---every
...
(放下)---he had no sword to put down. It is confirmation:
...
\medskip
\emph{He organised the people into groups of five and ten households,
...
fortunes, dispersed two---ensuring $\Ur$ never crossed the sword
threshold.
...
threshold, Fan Li reset it. The dollar is a sword precursor
...
(\cref{thm:dumu}), the pawn became a sword.
...
$\Ur$, you move from ``sword'' to ``hidden sword''---still dangerous,
...
sovereign) and capsizes the boat ($w = 0 \implies$ pawn $\to$ sword
...
\section{飞鸟尽良弓藏：韩信之误读}\label{app:proverb}
...
\begin{itemize}
  ...
  \item \textbf{Pre-sword} ($\Ur \neq \varnothing$,
  $r \notin \mathrm{Im}(\Obs)$): Xiao He.
  Imprisoned, then released. A dog that hid its teeth---the teeth
  remain.
\end{itemize}
...
Han Xin needed the third strategy (put down the sword). He chose the
...
\section{阿房宫赋}\label{app:afanggongfu}

Du Mu (杜牧, 803--852~CE) wrote the \emph{Rhapsody on the Epang
Palace} in 825~CE. \Cref{thm:dumu} formalises its central argument.
...
emperor's favour. Some waited thirty-six years and never saw him.}
\end{quote}
...
depleted their own water (internal sword dynamics, coordination cost
$O(n^2)$). 「族秦者秦也」$=$ $w(t) \to 0 \implies \text{pawn} \to
\text{sword} \implies \text{king absorbed}$. The causal chain is
internal.
