\section{第二性 --- The Other as Phase Function}\label{app:secondsex}
...
Knife $=$ mean     & exchange threshold             & Subject/Other boundary \\
\bottomrule
\end{tabular}
\end{center}

\subsection{The eighteen beats}
...
\begin{tikzpicture}[scale=0.85]
  ...
  \foreach \i in {0,...,17} {
    ...
    % Bottom: immanence (knife colour)
    \fill[knife!20] (\x, 0) rectangle (\x+\barw, \splitY);
  ...
  \node[above, font=\scriptsize, knife] at
...
\caption{The eighteen beats as narrative arc.
  Bottom (\textcolor{knife}{red}): immanence.
  Top (\textcolor{water}{blue}): transcendence.
  The melody line traces the Subject/Other boundary---the knife as
...
\subsection{The second sex is the mean}
...
The boundary between Subject and Other is not a property of 蔡文姬; it
is a phase function of the system's mean field.
When the mean actuation level shifts (war $\to$ peace, capture $\to$
ransom), the \emph{same person} crosses the threshold.
