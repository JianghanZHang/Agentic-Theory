...
the sword-is-the-mean theorem (\cref{thm:meanfield}) instantiates
...
The bureaucratic term 一刀切 (``one cut of the sword'') for
age-based cutoffs is a literal sword in the sense of
\cref{def:sword}: the age variable has autonomous dynamics
($a' = 1$, always advancing) and is fully observable.  It
satisfies both conditions of the sword definition.  Unlike other
swords in the framework, this one is not a phase function of the
mean field---it is an exogenous, non-negotiable constraint imposed
by temporal linearity itself.  Time is the sword that needs no
...
\begin{remark}[The sword is the mean guanxi level]
\label{rem:guanxi-sword}
By \cref{thm:meanfield}, the sword threshold is determined
...
This is the sword-is-the-mean theorem at county scale.
...
exogenous and fully observable (the time-sword of
...
\begin{proposition}[The secretary as delegated sword]
\label{prop:secretary-sword}
The secretary (秘书) position satisfies the sword definition
(\cref{def:sword}):
\begin{enumerate}[label=(\roman*)]
...
\end{enumerate}
Whether this sword is a tool or a threat is a phase function
...
mean-field sword threshold even when their individual
...
cadres are assigned to which edges.  The sword---the
...
capacity constraints.  The sword is the mean.
\end{remark}
