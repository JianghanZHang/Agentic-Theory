...
$L = \bigsqcup_{k=0}^{K} L_k$ with rank function
$\rho \colon L \to \{0, 1, \ldots, K\}$, equipped with:
\begin{enumerate}[label=(\roman*)]
  \item \textbf{Level sizes} $n_k := |L_k|$;
  ...
  \item \textbf{Time constants} $\tau_k > 0$: the mean duration
    at level $L_k$ before promotion to $L_{k+1}$
    (averaged over those who are promoted);
  \item \textbf{Age boundaries} $a_k^{\max}$: hard upper bounds
    on the age at which a cadre in $L_k$ remains eligible
    for promotion to $L_{k+1}$.
\end{enumerate}
\end{definition}
...
\noindent
The entry time (from first assignment to $L_0$) averages $8$ years.
A cadre entering at age $22$ therefore reaches $L_0$ at
${\sim}30$, $L_1$ at ${\sim}33$, $L_2$ at ${\sim}40$,
$L_3$ at ${\sim}48$.  The margin between this ideal trajectory
and the age boundary at $L_3$ is ${\sim}7$ years---thin enough
that a single delay of one promotion cycle can be fatal.
The lattice has additional substructure: each level contains
\emph{hidden steps} (隐形台阶), e.g.\ within $L_2$ the
...
and $r_0 < 1$.  The cumulative survival ratio from $L_0$ to
$L_3$ is
\[
  p_{\textup{top}}
  \;=\; \prod_{k=0}^{K-1} r_k
  ...
  \;\approx\; 0.0074.
\]
Fewer than $1\%$ of cadres at $L_0$ will occupy $L_3$ in their
...
$v$ reaches $L_{k+1}$, then $v$ is a cradle if
...
satisfying:
\begin{enumerate}[label=(\roman*)]
  \item every node in $C$ is a cadre at $L_k$ for some
    $k \geq 0$;
