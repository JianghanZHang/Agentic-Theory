...
def run_vacuum_story() -> None:
    ...
    gap_open = sys_open.mass_gap()
    print(f"  {_G}Phase 2 — GAP OPEN{_0} (μ = {mu_open})")
    print(f"    Δ = {gap_open.delta:.4f}  (mass gap protects L4)")
    print(f"    C_J(L1) = {gap_open.C_L1:.4f},  C_J(L4) = {gap_open.C_L4:.4f}")
    print(f"    L4 stable: {_G}{gap_open.L4_stable}{_0}")
    ...
    gap_closed = sys_closed.mass_gap()
    print(f"  {_R}Phase 3 — GAP CLOSED{_0} (μ = {mu_closed})")
    print(f"    Δ = {gap_closed.delta:.4f}  (energy gap still positive)")
    print(f"    C_J(L1) = {gap_closed.C_L1:.4f},  C_J(L4) = {gap_closed.C_L4:.4f}")
    print(f"    L4 stable: {_R}{gap_closed.L4_stable}{_0}")
