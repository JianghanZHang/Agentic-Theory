...
As $\varepsilon \to 0$, Varadhan's small-time
asymptotics~\cite{Varadhan1967} (applied in the local chart
$\exp\colon\mathfrak{g} \to G$, where cut-locus paths contribute
exponentially vanishing weight) shows
...
equation on $\mathfrak{g}$; barrier and tracking costs enter as
external forcing on the right-hand side
($\dot\mu = \mathrm{ad}^*_\xi\mu + f_{\mathrm{ext}}$).
\end{remark}

\begin{remark}[Hopf--Cole and barrier-augmented costs]\label{rem:hopf-cole-barrier}
The Hopf--Cole substitution $\psi = e^{-V/(2\varepsilon)}$ requires $V$ to be globally defined; it sends $V \to \infty$ to $\psi \to 0$. In the barrier-augmented formulation of \S\ref{sec:articulated}, where $V$ includes the log-barrier term $-\varepsilon\log h \to \infty$ as $h \to 0$, the linearized PDE has $\psi \to 0$ on the constraint boundary. The Feynman--Kac representation remains valid: paths hitting the barrier contribute zero weight ($\psi = 0$), which is the correct probabilistic interpretation---forbidden trajectories are suppressed rather than excluded by hard constraints.
\end{remark}
