...
% Allow slight extra inter-word stretch to reduce overfull boxes
\emergencystretch=1.5em

% Prevent vertical stretching on pages with short content (e.g., before large floats)
\raggedbottom

% Color annotations for algorithm
\definecolor{AcqColor}{named}{MidnightBlue}   % acquisition / exploration
\definecolor{PostColor}{named}{OliveGreen}     % GP posterior update
\definecolor{ConColor}{named}{BrickRed}        % contraction / constraint
\definecolor{CycleColor}{named}{Violet}        % cycle connection M→A→Â→M

% Suppress hyperref warnings from math in PDF bookmark strings
\pdfstringdefDisableCommands{%
  \def\tfrac#1#2{#1/#2}%
  \let\Rpos\relax \def\Rpos{R>0}%
  \let\R\relax \def\R{R}%
  \let\Z\relax \def\Z{Z}%
  \let\bT\relax \def\bT{T}%
  \let\cN\relax \def\cN{N}%
  \let\Htwo\relax \def\Htwo{H2}%
  \let\mathfrak\relax \def\mathfrak#1{#1}%
}
\hypersetup{
  pdftitle={Optimal Cycle Theory},
  pdfauthor={Jianghan Zhang},
}
...
\address{Department of Computer Science, Dartmouth College, Hanover, NH 03755}
...
\input{sections/lca-groups}
\input{sections/additive-clt}
\input{sections/multiplicative-clt}
\input{sections/fourier-mellin}
\input{sections/gaussian-measures}
\input{sections/heat-semigroup}
\input{sections/ergodic}
\input{sections/optimal-cycle}
\input{sections/hardy-spaces}
\input{sections/half-shift}
\input{sections/spectral-decomp}

% Part II: Lie Group Control
\input{sections/lie-group-intro}
\input{sections/compact-groups}
\input{sections/noncompact-groups}
\input{sections/hjb}
\input{sections/mppi}
\input{sections/forward-only}
\input{sections/riccati}
\input{sections/articulated}
\input{sections/chu-duality}

% Concluding
\input{sections/concluding}
